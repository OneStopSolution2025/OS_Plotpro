...
export default function SaleAgreement() {
  ...
  if (!data) return <div className="p-6 text-ink/50 text-sm">Loading agreement...</div>
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-2xl mx-auto p-10 font-sans text-ink">
        <div className="border-b-2 border-ink pb-4 mb-6">
          ...
          <p className="text-xs text-ink/50 font-mono mt-1">Booking #{bookingId.slice(0, 8)} · {new Date(data.booking.created_at).toLocaleDateString('en-IN')}</p>
        </div>

        <div className="grid grid-cols-2 gap-6 mb-6">
          <div>
            <p className="text-xs uppercase tracking-wide text-ink/40 mb-1">Buyer</p>
            <p className="font-medium">{data.customer.full_name}</p>
            <p className="text-sm text-ink/60 font-mono">{data.customer.phone}</p>
            {data.customer.address && <p className="text-sm text-ink/60">{data.customer.address}</p>}
          </div>
          <div>
            <p className="text-xs uppercase tracking-wide text-ink/40 mb-1">Project</p>
            <p className="font-medium">{data.project?.name}</p>
            <p className="text-sm text-ink/60">{data.project?.location}</p>
          </div>
        </div>
